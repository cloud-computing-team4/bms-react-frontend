...
export const useMails = (isModalOpen) => {
  ...
  const deleteMail = useCallback((id) => {
    axios
      ...
      .catch((e) => {
        toast.error(e.response?.data?.message ?? "메일 삭제 요청 실패");
      });
  }, []);
  ...
  const updateMail = useCallback((id, title, content) => {
    axios
      ...
      .catch((e) => {
        toast.error(e.response?.data?.message ?? "메일 수정 요청 실패");
      });
  }, []);
  ...
  useEffect(() => {
    ...
    if (mails.length === 0) {
      axios
        .get(`/mails`)
        .then((r) => {
          toast.success(r.data.data.length + "개 불러오기 성공");
          setMails(r.data.data);
        })
        .catch((e) => {
          toast.error(
            e.response?.data?.message ?? "전체 메일 불러오기 요청 실패",
          );
